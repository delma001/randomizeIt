# Unicode for Greek Letter mu "\u03bc"
# Unicode for Greek Letter sigma "\u03c3"
# Unicode for less than sign "\u003c"
# Unicode for equals sign "\u003d"
# Unicode for greater than sign "\u003e"
# Unicode for not equals sign "\u2260"
# The req() function is very handy for catching when
# user makes a numerInput field blank
...
shinyApp(
  ...
  # The dashboard provides a space to setup the randomization method.
  ui = dashboardPage(
    dashboardHeader(),
    ...
    dashboardSidebar(
      ...
      fileInput(inputId = "data_file", label = "Choose CSV File",multiple = FALSE,
                accept = c("text/csv", "text/comma-separated-values,text/plain", ".csv")),
      radioButtons(inputId = "responseType", label = h3("Response Type"), inline = TRUE,
                   choices = c("Quantitative", "Categorical"), select = "Quantitative"),
      selectInput(inputId = "response_var", label = "Response Variable", choices = "NONE"),
      conditionalPanel(condition = "input.response_var != 'NONE' & input.responseType == 'Categorical'",
                       radioButtons(inputId = "respRefGroup", label = h3("Reference Group"), inline = TRUE,
                                    choices = c("One", "Two"), select = "Two")
      ),
      selectInput(inputId = "explain_var", label = "Explanatory Variable", choices = "NONE"),
      conditionalPanel(condition = "input.explain_var != 'NONE'",
                       radioButtons(inputId = "explRefGroup", label = h3("Reference Group"), inline = TRUE,
                                    choices = c("One", "Two"), select = "Two")
      ),
      # Horizontal line ----
#      tags$hr(),
      ...
      # Type of Simulation
      radioButtons(inputId = "simType", label = h3("Type of Simulation"),
    ...
    dashboardBody(
      fluidPage(
        fluidRow(
          column(1, offset = 0,
                 numericInput(inputId = "numberSamples", label = "Number of Samples", 
                              min = 1, max = 5000, value=1)
          ),
          column(1, offset = 0,
                 actionButtonStyled("addMore", "+ SAMPLES", class="btn btn-success")
                 ),
          column(1, offset = 0,
                 actionButtonStyled("resetDist", "RESET", class="btn btn-danger")
                 ),
          column(2, offset = 0,
                 radioButtons(inputId = "normalOutline", label = "Normal Outline", 
                              choices = c("ON" = "TRUE", "OFF" = "FALSE"), 
                              selected = "FALSE", inline = TRUE)
          ),
          conditionalPanel(condition = "input.simType == 'CI'",
                           column(1, offset = 0,
                                  numericInput(inputId = "confLevel", label = "Confidence Level (%)", 
                                               min=1, max = 99, value=95)
                                  )
                           ),
          conditionalPanel(condition = "input.simType == 'TEST'",
                           column(1, offset = 0, 
#                                  numericInput(inputId = "nullMean", label = "\u03bc =", value=0),
          ...
          column(4, offset = 0,
                 radioButtons(inputId = "distTail", label = "Confidence Interval Tail: Percentile Method", 
                              choices = list("NONE", "Left", "Two-Tail", "Right"), 
                              selected = "NONE", inline = TRUE)
                 ),
          conditionalPanel(condition = "input.simType == 'TEST' & input.explain_var != 'NONE'",
                           column(2, offset = 0,
                                  radioButtons(inputId = "randomMethod", label = "Randomization Method", 
        ...
        fluidRow( 
          column(8, offset = 0, plotOutput(outputId = "sampleMeanPlot", 
                                           hover = hoverOpts(id = "meanPlotHover", 
                                                             delayType = "debounce", delay = 25), 
                                           width = "100%", height = 600)
                 ),
          column(4, plotOutput(outputId = "observedPlot", width = 'auto', height = 300)
                 ),
          column(4, plotOutput(outputId = "resamplePlot", width = 'auto', height = 300)
                 )
          ),
  ...
  server = function(input, output, session){
    ...
    set.values = function() {
      ...
      if(input$response_var != "NONE" & input$responseType == "Quantitative"){
        ...
        if(input$explain_var == "NONE"){ # One-sample Quantitative
          ...
          v = do(input$numberSamples) * resample(responseVar)
          ...
          if(input$simType == "CI"){ # Two-Sample CI Quantitative
            ...
            v1 = do(input$numberSamples) * resample(theData)
            mean1 = rowMeans(v1)
            
            theData = subset(responseVar, explainVar == levels(explainVar)[2])
            v2 = do(input$numberSamples) * resample(theData)
            mean2 = rowMeans(v2)
        ...
        if(nrow(states$values) > 0){
          ...
          if(input$response_var != "NONE" & input$responseType == "Categorical"){
            if(input$explain_var == "NONE"){ # One-Sample
              ...
              updateNumericInput(session, "nullMean", value = 1 - theNullValue)
    ...
    dotPlotMeans <- function (x, sampleMean = 0,
      ...
      if (mw <= top) {
        ...
        # CONFIDENCE INTERVAL
        if(input$simType == "CI" & req(input$confLevel)){
...
#                rightTail = paste("p1 - p2 >", input$nullMean)
    ...
    set.null.label = function(){
      if(input$explain_var == "NONE"){
        if(input$responseType == "Quantitative"){
          updateNumericInput(session, "nullMean", label = "\u03bc \u003d", value = 0, step = 1)
        } else{
          updateNumericInput(session, "nullMean", label = "p \u003d", 
                             value = 0.5, min = 0, max = 1, step = 0.01)
        }
      } else{
        if(input$responseType == "Quantitative"){
          updateNumericInput(session, "nullMean", label = "\u03bc1 - \u03bc2 \u003d", 
                             value = 0, min = 0, max = 0, step = 0)
        } else{
          updateNumericInput(session, "nullMean", label = "p1 - p2 \u003d", 
                             value = 0, min = 0, max = 0, step = 0)
        }
      }
    ...
    set.selected.tail = function(){
      ...
      updateRadioButtons(session, inputId = "distTail", selected = tailChoices[states$selectedTail], 
                         label = tailLabel,
...
# OBSERVERS for Reactive Objects
    ...
    observeEvent(input$resetDist, {
      states$reset = TRUE
      states$resample = TRUE
      states$sampleSelect = 1
    })
    
    observeEvent(input$distTail,{
      if(input$simType == "TEST"){
        if(input$explain_var == "NONE"){
          if(input$responseType == "Quantitative"){
           tailChoices = c("NONE", paste("\u03bc \u003c", input$nullMean), 
    ...
    observeEvent(input$respRefGroup, {
      states$doNotReset = TRUE
      reset.values()
    })
    
    observeEvent(input$explRefGroup, {
      states$doNotReset = TRUE
      reset.values()
    })
    ...
    observeEvent(input$responseType, {
      ...
      states$sampleSelect = 1
      if(input$simType == "CI"){
        updateRadioButtons(session, inputId = "distTail", selected = "NONE", 
    ...
    observeEvent(input$numberSamples, {
      req(input$numberSamples)
      if(input$numberSamples > 10000){
        updateNumericInput(session, "numberSamples", value = 10000)
      }
    })
    ...
    observeEvent(input$confLevel, {
      ...
      if(input$confLevel < 1){
        updateNumericInput(session, "confLevel", value = 1)
      }
      if(input$confLevel > 99){
        updateNumericInput(session, "confLevel", value = 99)
      }
    })
    
    observeEvent(input$nullMean, {
        if(input$simType == "TEST"){
          if(input$explain_var == "NONE"){
            ...
            if(input$nullMean != 0){
              updateNumericInput(session, "nullMean", value = 0)
            }
          }
          set.selected.tail()
          states$doNotReset = FALSE
    ...
    observeEvent(input$randomMethod, {
      states$reset = TRUE
      states$resample = TRUE
      states$sampleSelect = 1
    })
    ...
    df <- reactive({
      req(input$data_file)
      read.csv(input$data_file$datapath, as.is = F)
      })
    ...
    observe({
      ...
      if(input$responseType == "Quantitative"){
        updateSelectInput(session, "response_var", choices = c("NONE", response_list))
      }
      if(input$responseType == "Categorical"){
        updateSelectInput(session, "response_var", choices = c("NONE", explain_list))
      }
      updateSelectInput(session, "explain_var", choices = c("NONE", explain_list))
      updateRadioButtons(session, inputId = "distTail", selected = "NONE")
    })
    ...
    observeEvent(input$response_var,{
      ...
      if(input$response_var != "NONE" & input$responseType == "Categorical"){
        ...
        updateRadioButtons(session, inputId = "respRefGroup", choices = response_levels, 
    ...
    observeEvent(input$explain_var,{
      states$values = NULL
      if(input$explain_var != "NONE"){
        explainPlace = which(names(df()) == input$explain_var)
        explain_levels = levels(df()[, explainPlace])
        updateRadioButtons(session, inputId = "explRefGroup", choices = explain_levels, 
    ...
    output$resamplePlot <- renderPlot({
      ...
      if(input$response_var != "NONE"){
        thePoints = nearPoints(states$meanPoints, input$meanPlotHover, 
                               xvar = "x", yvar = "y")
